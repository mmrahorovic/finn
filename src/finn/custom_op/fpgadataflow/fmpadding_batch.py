...
class FMPadding_Batch(HLSCustomOp):
    ...
    def get_nodeattr_types(self):
        my_attrs = {
            ...
            # Current padding scheme that is applied tries to pad the same amount of zeros in front and behind the image for each dimension.
            # As an example, padding schemes such as [1, x, 3, x] are equal to [2, x, 2, x]
            "Padding": ("ints", True, [1, 1, 1, 1]), # [H_begin, W_begin, H_end, W_end] = [Y_begin, X_begin, Y_end, X_end]
    ...
    def defines(self, var):
        ...
        if is_square:
            assert(pad_h == pad_w), "Only equal padding along the dimensions for square images is supported"
    ...
    def docompute(self):
        ...
        if is_square:
            ...
            self.code_gen_dict["$DOCOMPUTE$"] = [
                """{}<OutputDim1_x, OutputDim1_y, Padding1_x, Padding1_y, NumChannels1, SIMD1, {}, PaddingStyle1> (in0, out, numReps);""".format(
                    hls_call, in_t
                )
            ]
